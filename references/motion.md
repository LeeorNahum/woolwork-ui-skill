...
- The enhanced select keeps a real native select for form submission and no-JavaScript fallback. Its aligned, rounded paper options unfold from one recessed felt pocket with alternating static fold faces and an opaque, transform-only paper backing. The backing prevents nearby copy from showing through between moving folds. Closing runs the same fold sequence in exact reverse without moving surrounding layout.
