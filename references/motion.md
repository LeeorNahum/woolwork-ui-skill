...
- The enhanced select keeps a real native select for form submission and no-JavaScript fallback. Its visible control is one properly sized paper sheet, sized and aligned like the pocket inputs beside it; its aligned, rounded paper options unfold directly from its own bottom edge with alternating static fold faces and an opaque, transform-only paper backing. The backing prevents nearby copy from showing through between moving folds. Closing runs the same fold sequence in exact reverse without moving surrounding layout.
