...
| `references/dictionary.md` | The element-to-craft mapping. Every kit component must have a row here, and every row must correspond to a real class in the kit |
...
- Quote every frontmatter string value in `SKILL.md`. Keys stay unquoted. No semicolons used to join what should be separate sentences in hand-authored prose, only in genuine list separators or literal CSS/code.
...
- All turbulence is baked into data-URI tiles, with no runtime SVG filters.
